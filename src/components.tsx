...
export const antdComponents = {
  Card: ({
    ...
    return (
      <Card
        ...
        extra={extraSlot ?? (props.extra ? <span>{props.extra}</span> : undefined)}
        variant={props.variant ?? "outlined"}
        hoverable={props.hoverable ?? false}
        loading={props.loading ?? false}
        size={props.size ?? "default"}
        cover={coverSlot ?? (props.cover ? <img src={props.cover} alt="cover" /> : undefined)}
        style={{
  ...
  Grid: ({ props, children }: BaseComponentProps<AntdProps<"Grid">>) => {
    ...
    // Use custom spans if provided, otherwise calculate from columns
    const getSpan = (index: number): number => {
      if (props.spans && props.spans[index] !== undefined) {
        return props.spans[index];
      }
      const columns = props.columns ?? 3;
      return Math.floor(24 / Math.min(Math.max(columns, 1), 24));
    };

    return (
      <Row gutter={[gutter, gutter]}>
        {childArray.map((child, i) => (
          <Col key={i} span={getSpan(i)}>
            {child}
          </Col>
        ))}
      </Row>
  ...
  Divider: ({ props }: BaseComponentProps<AntdProps<"Divider">>) => {
    return (
      <Divider
        orientation={props.vertical ? "vertical" : (props.orientation ?? "horizontal")}
  ...
  Breadcrumb: ({ props }: BaseComponentProps<AntdProps<"Breadcrumb">>) => {
    ...
    return (
      <Breadcrumb separator={props.separator ?? "/"} items={items} />
    );
  },
  ...
  Dropdown: ({
    ...
    const isControlled = bindings?.open !== undefined || props.open !== undefined;
  ...
  Table: ({ props }: BaseComponentProps<AntdProps<"Table">>) => {
    ...
    return (
      <Table
        ...
        scroll={props.scroll ? { x: props.scroll.x ?? undefined, y: props.scroll.y ?? undefined } : undefined}
  ...
  Image: ({ props, emit, on }: BaseComponentProps<AntdProps<"Image">>) => {
    // When click event is bound, disable preview to avoid conflict
    // Otherwise use props.preview or default to false
    const hasClickHandler = on("click").bound;
    const preview = hasClickHandler ? false : (props.preview ?? false);
  ...
  Tree: ({
    ...
    bindings,
    emit,
  }: BaseComponentProps<AntdProps<"Tree">>) => {
    ...
    const checkedKeys = isBoundChecked ? (boundCheckedKeys ?? []) : localCheckedKeys;
    const setCheckedKeys = isBoundChecked ? setBoundCheckedKeys : setLocalCheckedKeys;
    ...
    const transformTreeData = (data: unknown[]): unknown[] => {
      return data.map((item: any) => ({
        key: item.key,
        title: item.title,
        ...(item.children ? { children: transformTreeData(item.children) } : {}),
      }));
    };
    ...
    return (
      <AntLink
        ...
        onClick={(e) => {
          ...
          if (click.shouldPreventDefault || !props.href || props.href === "#") {
            e.preventDefault();
          }
          click.emit();
        }}
